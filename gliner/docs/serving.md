...
curl --fail http://localhost:8080/health-check
```

Both endpoints return HTTP 200 with `{"status": "ok"}` once the model deployment
is ready. Docker uses `/health` for its built-in health check. The container's
route prefix defaults to `/` so both health endpoints are accessible at the root;
keep this setting when using these health checks. Predictions remain available
at `POST /gliner`. For the Python client, use
`GLiNERClient(base_url="http://localhost:8080")`.

Alternatively, run `docker compose up --build` from the same directory.
For a custom model, use
`GLINER_MODEL=urchade/gliner_medium-v2.1 docker compose up --build`.
